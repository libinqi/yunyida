...
app.controller('infoAuditAuditInfoCtrl', ['$scope', '$http', 'dialog', 'infoAuditService','Upload', function ($scope, $http, dialog, infoAuditService,Upload) {
    var vm = this;
    ...
    vm.getEnterpriseInfo = function(){
      infoAuditService.getEnterpriseInfo($scope.eid).then(function (response) {
    ...
    vm.auditInfo = function(status){
        if($scope.myForm.$valid){
          vm.auditJson.enterpriseid = $scope.eid;
          vm.auditJson.status = status;
          vm.auditJson.auditUserId = user.userid;
          // vm.auditJson.auditRemark = vm.jsondata.auditRemark;
